...
wb <- openxlsx::loadWorkbook("Results/Education_Productivity_Gap_Ghana_Results_smf.xlsx")
openxlsx::writeData(wb, sheet = "msf",res , colNames = T, startCol = "A", startRow = 1)
openxlsx::saveWorkbook(wb,"Results/Education_Productivity_Gap_Ghana_Results_smf.xlsx",overwrite = T)
#-----------------------------------------
# Fig - Heterogeneity                  ####
rm(list= ls()[!(ls() %in% c(Keep.List))])
...
eff_fig_fxn <- function(disasg,type=NULL,xsize=7,title=""){
  ...
  write.csv(data[order(data$input),c("input","disasg","level","Estimate","Estimate.sd","jack_pv")],
            file=paste0("Results/FigsData/",paste0(disasg,collapse = "_"),".csv"))
  return(fig)
}

grobs <- ggplotGrob(eff_fig_fxn(disasg = "CROP",xsize=5.5,title="(A) Major crops")+theme(legend.position="bottom"))$grobs
legend <- grobs[[which(sapply(grobs, function(x) x$name) == "guide-box")]]
Ylab<-ggplot()+geom_text(aes(x=0,y=0),label="Percentage Difference (Educated less Uneducated)\n",size=3,angle=90)+theme_void()
...
fig.Farmer   <- eff_fig_fxn(disasg = c("AgeCat","Female","EduLevel"),
                            type="farmer",xsize=7) +
  labs(title="",x="", y ="Percentage Difference (Educated less Uneducated)\n",caption = "") +
  theme(legend.position="bottom")
if(Sys.info()['sysname'] =="Windows"){ggsave("Results/Figs/heterogeneity_genderAge.png", fig.Farmer,dpi = 600,width = 5, height = 5)}
if(Sys.info()['sysname'] !="Windows"){ggsave("Results/Figs/heterogeneity_genderAge.pdf", fig.Farmer,dpi = 600,width = 5, height = 5)}
#-----------------------------------------
# Fig - Robustness                     ####
rm(list= ls()[!(ls() %in% c(Keep.List))])
data <- as.data.frame(
...
fig <- ggplot(data=dataF,aes(x=factor(x), y=Estimate, group=1)) +
  ...
  labs(title= "", x = "", y = "\nDifference (%) [Educated less Uneducated]", caption = "") +
  ers_theme() +
  theme(axis.title= element_text(size=9,color="black"),
        plot.title  = element_text(size = 8),
