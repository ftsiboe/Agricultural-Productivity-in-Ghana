...
data$Matching <- ifelse(data$name %in% "mahalanobis" ,3,data$Matching)
data$Matching <- ifelse(data$name %in% "robust_mahalanobis" ,4,data$Matching)
data$Matching <- ifelse(data$name %in% "logit"  ,5,data$Matching)
data$Matching <- ifelse(data$name %in% "probit" ,6,data$Matching)
data$Matching <- ifelse(data$name %in% "cauchit" ,7,data$Matching)
data$Matching <- ifelse(data$name %in% "cloglog" ,8,data$Matching)

data$Matching <- factor(
  data$Matching,levels = 1:8,
  labels =c("Euclidean",
            "Euclidean - scaled",
            "Mahalanobis]",
            "Mahalanobis- scaled",
...
data <- data[!data$outcome %in% c("Credit","Eqip","Extension","OwnLnd"),]

data$outcome <- as.numeric(
  as.character(factor(data$outcome,
                      levels = c("HrvstKg","Area", "SeedKg", "HHLaborAE","HirdHr","FertKg","PestLt"),labels = 1:7)))
data$outcome <- factor(data$outcome,levels = 1:7,
                       labels = c(
                         "Output (real value)","Land (ha)", "Planting materials (real value)", "Household labor (AE)","Hired labor (man-days/ha)","Fertilizer (Kg/ha)",
                         "Pesticide (Liter/ha)"))

figTe <- ggplot(data=data,aes(x=Matching, y=(exp(estimate)-1)*100)) +
  geom_hline(yintercept =0,size = 0.5,color = "black") +
  geom_errorbar(aes(ymax = (exp(conf.high)-1)*100 , ymin = (exp(conf.low)-1)*100), width = 0.25,colour="blue") +
  geom_point(colour="blue",pch=21, size=2,fill="purple") +
  labs(title="", x="\nMatching matrix method",
       y = "Average treatment effect of disability (%)\n",caption ="") +
  ...
  theme(legend.text=element_blank(),
        ...
        axis.title.y=element_text(size=8),
        axis.title.x=element_text(size=8),
        axis.text.x = element_text(size = 8,vjust=0.5,angle = 90, colour="black"), #
        axis.text.y = element_text(size = 8, colour="black"),
        plot.caption = element_text(size=10, hjust = 0 ,vjust =0, face = "italic"),
        strip.text = element_text(size = 11),
...
dataFrq$Tech <- factor(as.numeric(as.character(dataFrq$TCHLvel)),levels = 0:1,labels = c("Not disabled","Disabled"))
...
eff_fig_fxn <- function(disasg,type=NULL,xsize=7,title=""){
  ...
  ggplot(data=data,aes(x = x,y=Estimate ,group=input,shape=input,colour=input,fill=input)) +
...
fig <- ggplot(data=dataF,aes(x=factor(x), y=Estimate, group=1)) +
  ...
  labs(title= "", x = "", y = "\nDifference (%) [Disabled less non-Disabled]", caption = "") +
  ers_theme() +
  theme(axis.title= element_text(size=9,color="black"),
        plot.title  = element_text(size = 8),
